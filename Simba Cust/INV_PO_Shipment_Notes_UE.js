 ...
 * 2.00       01 Aug 2024     Abhay Ambale        Old notes checkbox for PO Shipment Notes, Line level reference of PO shipment note on Purchase Order
 ***********************************************************************/
define(['N/record', 'N/search', 'N/format', 'N/log'], function (record, search, format, log) {
    ...
    function _afterSubmit(context) {
        ...
        var formName =curRec.getValue({fieldId:'customform'});
        log.debug("Form Name: ", formName);
        if(formName && formName=='187'){
            try {
                if (recordMode == 'CREATE' || recordMode == 'EDIT') {
                    ...
                    // Mark old Note checkbox for existing records for changed dates
            ...
            Object.keys(existingDate).forEach(function(date) {
                ...
                for (var i = 0; i < numLines; i++) {
                    ...
                    if (fieldValue == date) {
                        ...
                        log.debug("Setting the shipment note value in sublist",existingDate[date]);
            ...
            try{
                ...
                for (var i = 0; i < lineCount; i++) {
                    ...
                    // Inactivating PO Notes if Container Number from PO shipment note and SHipment Headers Matches.
                    var shipmentHeader = curRec.getSublistValue({
                        sublistId: 'item',
                        fieldId: 'custcol_kl_shipment_header',
                        line: i
                    });
                    var poShipmentNote = curRec.getSublistValue({
                        sublistId: 'item',
                        fieldId: 'custcol_shipment_note_ref',
                        line: i
                    });
                    //customrecord_po_shipment_notes , customrecord_os_shiphead                
                    if(shipmentHeader && poShipmentNote){
                        if(poShipmentNote == tempPOShipmentNote){
                            continue;
                        }
                        tempPOShipmentNote = poShipmentNote;

                        var shipmentHeaderRec = record.load({
                            type: "customrecord_os_shiphead",
                            id: shipmentHeader,
                        });

                        var poShipmentNoteRec = record.load({
                            type: "customrecord_po_shipment_notes",
                            id: poShipmentNote,
                        })
                        
                        var shContainer = shipmentHeaderRec.getValue({fieldId:'custrecord_os_sh_containernum'});
                        var poNoteContainer = poShipmentNoteRec.getValue({fieldId:'custrecord_inv_container_booking'});
 
                        if(shContainer==poNoteContainer){
                            log.debug("Container Matched ","Making PONote Inactive: "+poShipmentNoteRec);
                            poShipmentNoteRec.setValue({ fieldId: 'isinactive', value: true });
                        }
                        poShipmentNoteRec.save();
                    }
                }
            ...
            log.debug("Different form found: ", "Script can't execute due to different form");
        }
