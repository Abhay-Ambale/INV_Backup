...
function SetApprovalMatrix_BL(type, form, request) {
	...
	if(type == 'view' &&  execContext == 'userinterface') {
		...
		if(recType =='inventoryitem' || recType =='assemblyitem'){			
			...
			if(currAppStatus == PENDING_APPROVAL_AM || currAppStatus == PENDING_APPROVAL_FM){
				...
				nlapiLogExecution('debug','srchResults',srchResults.length);
				if(_validateData(srchResults) && srchResults.length > 0)
				{				
					var approvalStatus 	= srchResults[0].getValue('custrecord_inv_approval_status');
					...
					nlapiLogExecution('debug','approverRole2',approverRole2);
					...
					nlapiLogExecution('debug','approverRole2 indexOf',approverRole2.indexOf(currContext.getRole())); */
			...
			if(currAppStatus == PENDING_APPROVAL){
				...
				{					
					...
					// Buttons for Approver 1
					if(approverRole1.indexOf(currContext.getRole()) != -1){
						var poObj		= nlapiLoadRecord('purchaseorder', recId);
						var customform	= poObj.getFieldValue('customform');
						if(customform == FRM_PO_IC_MAGNUM) {
							var call_Suitelet = nlapiResolveURL("SUITELET", "customscript_inv_create_correspond_po_sl","customdeploy_inv_create_correspond_po_sl", null);
							call_Suitelet 	= call_Suitelet + '&pid='+recId;
							form.addButton("custpage_apprv", 'Approve',"window.open('"+call_Suitelet+"', '_self');");
						}else{
							var apprvStr = _getApproveStr('approvalstatus', APPROVED);
							form.addButton('custpage_apprv','Approve 2',apprvStr);
						}
						
						var rejectStr = _getRejectStr('approvalstatus', REJECTED);						
						form.addButton("custpage_reject",'Reject', rejectStr); 
					}
				}
			}
