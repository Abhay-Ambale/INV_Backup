	...
	if(type == 'view' &&  execContext == 'userinterface') {
		...
		/* else if(recType =='estimate'){
			...
			if(currAppStatus == PENDING_APPROVAL){
				...
				// Intercompany Account Type Id = 626
				if(_validateData(intercoSub))
					filters.push(new nlobjSearchFilter('custrecord_inv_po_account_type', null, 'anyof', 626));
				else
					filters.push(new nlobjSearchFilter('custrecord_inv_po_account_type', null, 'noneof', 626));
				...
				{					
					...
					// Buttons for Approver : Intercompany PO one level approver else two level
					if(_validateData(intercoSub)){
						if(customAppStatus == 1 && approverRole1.indexOf(currContext.getRole()) != -1){						
							var apprvStr 	= _getApproveStr3('custbody_inv_po_approval_status', 3);	// 1 level approval					
							form.addButton('custpage_apprv','Approve',apprvStr);					
						}
					}
					else {
						...
						if(customAppStatus == 1 && approverRole1.indexOf(currContext.getRole()) != -1){
							if(customform == '196' || customform == '198'){
								var apprvStr 	= _getApproveStr3('custbody_inv_po_approval_status', 3); // 1 level approval
							}
							else {
								if(_validateData(approverRole2))
									var apprvStr 	= _getApproveStr('custbody_inv_po_approval_status', 2); // 2 level approval
								else
									var apprvStr 	= _getApproveStr3('custbody_inv_po_approval_status', 3); // 1 level approval for usa
							}
							form.addButton('custpage_apprv','Approve',apprvStr);					
						}					
