...
function ItemFulfillment_BS(type){
	nlapiLogExecution('debug', '---------- ItemFulfillment_BS type', type);
	if(type == 'create'){
		var lineCount		= nlapiGetLineItemCount('item');
		for(var i = 1; i<=lineCount; i++) {
			var itemPrice = nlapiGetLineItemValue('item', 'custcol_inv_item_selling_price_dyn', i);
			//nlapiLogExecution('debug', 'itemPrice', itemPrice);			
			nlapiSetLineItemValue('item', 'custcol_inv_item_selling_price', i, itemPrice);			
		}
		
		// Set Order Version No
		var wmsOrderNo		= nlapiGetFieldValue('custbody_inv_wms_order_number');		
		if(!_validateData(wmsOrderNo)){
			var createdfrom		= nlapiGetFieldValue('createdfrom');
			var createdfromTxt	= nlapiGetFieldText('createdfrom');				
			var ifOrderNo 		= _getIForderNo(createdfrom, createdfromTxt);
			nlapiSetFieldValue('custbody_inv_wms_order_number', ifOrderNo);
		}
	}
}

function _getIForderNo(soId, soNo){
	var ifCount		= 0;	
						...
						[
						  ["type","anyof","SalesOrd"], 
						   ...
						   ["applyingtransaction.mainline","is","T"],
      						"AND", 
   						   ["location","noneof","12","15"]
						], 
						[
						    new nlobjSearchColumn("applyingtransaction",null,"COUNT"), 
							new nlobjSearchColumn("tranid",null,"GROUP")
						]
						);
	if(_validateData(soSearch) && soSearch.length > 0){		
		ifCount	= soSearch[0].getValue("applyingtransaction",null,"COUNT");		
	}
	ifCount		= Number(ifCount) + 1;
	soNo		= soNo.replace("Sales Order #", "");
	var orderNo = soNo+'.'+ifCount;
...
function ItemFulfillment_AS(type, form){
	...
	if(_validateData(recId) && (type=='create' || type=='copy' || type=='edit' || type=='ship'))
	{
		...
		if(createdfromType == 'SalesOrd'){
			...
			// Below code commented on 21st Nov 18 By Supriya (Invitra) as per discussion in today's call with Kelly : Simba didn't require this auto I/C Item receipt functionality.
