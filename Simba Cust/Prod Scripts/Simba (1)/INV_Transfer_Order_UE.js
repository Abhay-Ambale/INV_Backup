...
function TransferOrder_AS(type){
	if(type == 'create'){
		...
		var buildId		= nlapiGetFieldValue('custbody_inv_assembly_build_ref');
		if(_validateData(woId) && !_validateData(buildId)){
		...
		if(_validateData(buildId)){
			nlapiSubmitField('assemblybuild', buildId, 'custbody_inv_outsourced_to_ref', recId);			
		}
	}
}