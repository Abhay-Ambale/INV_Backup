...
function Item_BL(type, form) {
	...
	if((type == 'create' || type == 'copy') && execContext == 'userinterface'){		
		...
		form.getField('custitem_legacy_item_code').setDisplayType('inline');
		//form.getField('upccode').setDisplayType('disabled');
		//nlapiSetFieldValue('custitem_inv_auto_upc_code', 'T');
		nlapiSetFieldValue('custitem_inv_appproval_status', 1);
		
		nlapiSetFieldValue('upccode', '');
		nlapiSetFieldValue('custitem_legacy_item_code', '');
		nlapiSetFieldValue('custitem_inv_scan_pack_barcode', '');
		nlapiSetFieldValue('custitem_inv_carton_barcode', '');
		
		nlapiSetFieldValue('offersupport', 'T');	
	...
	if(type == 'edit' && execContext == 'userinterface'){		
		...
		/* if(isinactive == 'T' && _validateData(appprovalStatus) && appprovalStatus != APPROVED_ITEM){
			form.getField('isinactive').setDisplayType('disabled');
		} */
		
		form.getField('custitem_legacy_item_code').setDisplayType('disabled');
		if(_validateData(upccode) && autoUpcCode == 'T') {
			 form.getField('upccode').setDisplayType('disabled');
			 form.getField('custitem_inv_auto_upc_code').setDisplayType('disabled');
			 form.getField('custitem_inv_scan_pack_barcode').setDisplayType('disabled');
			 form.getField('custitem_inv_carton_barcode').setDisplayType('disabled');
		}
	}
	/* if(type == 'view'){
		var approvalSt		= nlapiGetFieldValue('custitem_inv_appproval_status');
		if(approvalSt == 1)
			form.setTitle('Item Pending Approval');
	} */
}
...
function Item_AS(type) {
	...
	if((type == 'create' || type == 'edit') && (exeContext == 'userinterface' || exeContext == 'csvimport')&& (recType == 'inventoryitem' || recType == 'assemblyitem')){
		...
		var oldExternalid 	= oldRecord && oldRecord.getFieldValue('externalid');
		...
		if(oldExternalid != itemid && recType == 'inventoryitem'){
			nlapiLogExecution('debug', 'externalid itemid', itemid);
			nlapiSubmitField(recType, recId, ['externalid'], [itemid]);
		}
			
		//if(type=='edit' && autoUpcCode == 'T' && oldUpccode != upccode){
        if(type=='edit' && autoUpcCode == 'T' && (oldUpccode != upccode || !_validateData(upccode)) ){
			//nlapiLogExecution('debug', 'recType inside', recType);
			_generateUpcCode(recId, recType, type);
		}
	}
}
function _generateUpcCode(recId, recType, type){
	...
	{					
		...
		if(_validateData(rawcode) && rawcode.length == 12) {
			...
			//nlapiLogExecution('debug', 'checkDigit', checkDigit);
